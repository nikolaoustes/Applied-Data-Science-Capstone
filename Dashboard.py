...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        filtered1 = filtered.groupby(['Launch Site', 'class']).size().reset_index(name='class count')
        fig = px.pie(filtered1, values='class count',
                     names='class', title='Success rate for ' + entered_site)
        return fig
